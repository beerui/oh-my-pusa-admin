        ...
        <div class="left-operation-container">
          <t-form>
            <t-form-item label-width="0">
              <t-input v-model="addForm.name" placeholder="请输入标签名称"></t-input>
...
import { tagAdd, tagDel, tagList, tagUpdate } from '@/api/app'
...
const fetchData = async () => {
  ...
  try {
    const { list, total } = await tagList({
...
const onConfirmRename = async () => {
  // 真实业务请发起请求
  if (chooseId.value !== -1) {
    await tagUpdate({ id: chooseId.value, name: addForm.name })
    fetchData()
